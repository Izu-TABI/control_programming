...
def Integral(y, h, ss, ef_bak):
  s = ss + h * (ef_bak + y) / 2.0
  ef_bak = y
  return (s, ef_bak)

def Integral_wr_wt(om):
  h = 0.5 * 10 ** -3
  ...
  for t in t_list:
    y = omr - om
    ss, ef_bak = Integral(y, h, ss, ef_bak)
    s.append(ss)
...
# 解くべき微分方程式1
def f_diadt(ia, om, Ki):
  diadt = (Ka * As * Kp * omr - Ra * ia - om * (Kv + Ka * As * Kp) + Ki * As * Ka * Integral_wr_wt(om)) / La
...
def plot_om(x, y1, y2, y3, x_label, y_label):
  ...
  sol.legend(['Kp=2.0', 'Kp=5.0', 'Kp=10.0'])
...
if __name__ == "__main__":
  ...
  for i in range(3):
    for t in t_list:
      ia_t, om_t = RungeKutta(ia[i][-1], om[i][-1], Ki[i], h) #yを求める
